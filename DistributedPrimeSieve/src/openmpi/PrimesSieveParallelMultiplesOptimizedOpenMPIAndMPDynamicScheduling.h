...
#				endif
		...
		void setupSegmentDistribution(size_t maxRange) {
			size_t numberSievingProcesses = _numberSievingProcesses;

			_segmentsDistribution = vector<vector<pair<size_t, size_t> > >(numberSievingProcesses, vector<pair<size_t, size_t> >());

			size_t maxRangeSquareRootOffset = (size_t) sqrt(maxRange) + 1;
			size_t numberSegmentsPerProcess = _dynamicSchedulingNumberSegments / numberSievingProcesses;

			size_t numbersegmentsAdded = 0;
			size_t dynamicSchedulingBlockSizeInElements = _dynamicSchedulingSegmentSizeInElements;

#			ifdef DEBUG_OUTPUT
			cout << "    > Initializing segment distribution in root node (" << _dynamicSchedulingNumberSegments << " segments in " << numberSievingProcesses << " processes)" << endl;
#			endif

#			pragma omp parallel for \
				default(shared) \
				firstprivate(numberSievingProcesses, numberSegmentsPerProcess, dynamicSchedulingBlockSizeInElements, maxRangeSquareRootOffset) \
				reduction(+: numbersegmentsAdded)
			for (size_t processPosition = 0; processPosition < numberSievingProcesses; ++processPosition) {
...
#					endif
			...
			size_t lastProcessNumberIndex = numberSievingProcesses - 1;
...
#				endif
		...
		void setupRootSegmentManagementTasks(size_t maxRangeSquareRoot, size_t numberSievingProcesses) {
...
#				pragma omp taskwait
		...
		void setupRootSegmentManagementSections(size_t maxRangeSquareRoot, size_t numberSievingProcesses) {
			size_t numberSegmentsCreated = _numberSegmentsCreated;
			vector<vector<pair<size_t, size_t> > >& segmentsDistribution = _segmentsDistribution;
...
#				endif
		...
		void startServerOfSegmentsAvailable(size_t numberSegmentsCreated, vector<vector<pair<size_t, size_t> > >& segmentsDistribution, size_t numberSievingProcesses) {
			...
			size_t numberManagementProcesses = this->template getNumberProcesses() - _numberSievingProcesses;
...
#					endif
		...
		void endSievingNodes(size_t numberSievingProcesses) {
...
#					endif
		...
		void sendSievedBlockDataToCollector(int processIDToSendResults) {
		...
		void startCollectorOfSievingDataResultsTask() {
			cout << "    > Collector for nodes segments results started..." << endl;
...
#			pragma omp for \
		...
		size_t startCollectorOfNodesPrimesCount(size_t numberSegmentsCreated) {
		...
		size_t receivePrimesCountFromNode(int nodeID) {
...
#			endif
		...
		inline int getMpiThreadSupport() const {
			return _mpiThreadSupport;
		}

		inline void setMpiThreadSupport(int mpiThreadSupport) {
